...
rotterdam_cleaned <- rotterdam |> 
        mutate(
                ...
                nodes_cat = factor(
                        ...
                        levels = c("0", "1-5", "6-10", "11+")),
                
                # Add dichotomized variable for number of nodes
                nodes_dich = factor(
                        case_when(nodes == 0 ~ "0",nodes >0 ~ "1+")),
...
survobj_d <- survival::Surv(time = rotterdam_cleaned$dtime, event = rotterdam_cleaned$death)
...
# rotterdam_surv_fit_d <-  survival::survfit(survobj_d ~ 1)
rotterdam_surv_fit_d <-  ggsurvfit::survfit2(survobj_d ~ 1)
